	...
	public static TileGraph generateGraph(TiledMap map) {
		...
		for (int i = 0; i < LevelManager.verticalTiles; i++) {
			for (int j = 0; j < LevelManager.horizontalTiles; j++) {
				...
				newNode.setPosition(new Vector2(j * LevelManager.tileWidth + LevelManager.tileWidth / 2,
		...
		if (other != null && !other.isObstacle()) {
			...
			if (!other.isObstacle()) {
				...
				if (iOffset != 0 && jOffset != 0) {
					// Se for uma diagonal, multiplicar o custo por raíz de dois
					n.createConnection(other, getCost(map, j + jOffset, i + iOffset) * 1.4142f);
				} else {
					// Se não for diagonal
